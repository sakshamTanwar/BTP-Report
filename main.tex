...
\section{Proposed Solution Architecture}
    ...
    \subsection{\gls{lrrs} Server}
        ...
        \paragraph{}
        The generated PDF contains general details about the land parcel on top and it also includes a Verification Code which can be used to verify the current owner of the land parcel using the Verification server and selecting the record type as Land Record. The PDF also contains a table of ownership history of the land parcel from genesis with the Verification Code for the Transfer Records which can also be verified on the verification server. These verification codes are the hash ID's of the records stored on the IPFS. To validate the land record and the transfer records, the users can request for these certificates from the verification portal.
        \begin{figure}[H]
