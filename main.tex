...
\section{Introduction}
    ...
    \subsection{Disadvantages of a centralized database}
        ...
        Access to this data is usually provided by a \gls{dbms} consisting of an integrated set of computer software that allows users to interact with one database and provides access to all of the data contained in the database.
        \gls{dbms}'s provide various functions that allow management of a database and its data such as :-
        \begin{itemize}
        ...
        \end{itemize}
        A database model is a type of data model that determines the logical structure of a database and fundamentally determines in which manner data can be stored, organized and manipulated. The most popular example of a database model is the relational model, which uses a table-based format. The \gls{acid} database design is one of oldest and most important database design. A relational database that fails to follow the \gls{acid} model cannot be considered reliable. 
        \paragraph{}
        Databases can be categorized in many categories depending on how they store data, where it is stored and the additional functionality the \gls{dbms} can provide on the stored data. Broadly databases can categorized into two major categories centralized and distributed. They both have their advantages and disadvantages.
        \paragraph{}
        A centralized database is a database that is located, stored, and maintained in a single location and has a single \gls{dbms} but can be accessed by the users distributed in the network.
        \paragraph{}
        ...
        For a \gls{dbms} to be distributed, it should be fully compliant with the twelve rules introduced by C.J. Date in 1987 \cite{distributeddbms} -:
        \begin{enumerate}
            \item Local Autonomy
    ...
    \subsection{\gls{ipfs}}
        ...
        \paragraph{}
        The \gls{ipfs} \cite{IPFS} is a peer to peer distributed file system for sharing and storing data. \gls{ipfs} provides a high throughput storage system where each file is uniquely identified by a cryptographic hash. In \gls{ipfs}, multiple users store a piece of each others' data and actively participate in making it available in the network. In contrast to HTTP which uses location based addressing, \gls{ipfs} uses content based addressing. In content based addressing, user request data using its content identifier instead of the location where it is stored.

        \paragraph{}
        \gls{ipfs} uses Distributed Hash Table (DHT) \cite{DHT} to enable routing and discovery of peers and content on the network. The hash table is split across all the participating nodes in the \gls{ipfs}. These nodes coordinate with each other to enable efficient lookup and retrieval of data on the network. \gls{ipfs} stores data using a data structure called Merkle DAGS \cite{IPFS}. Each node in a Merkle DAG stores the cryptographic hash of each of its children. Thus, altering a node's data changes the hash value of the node as well as all its ancestors in the DAG. This makes the data stored on the \gls{ipfs} immutable.
...
\section{Advantages of Decentralized Solutions}
    ...
    \subsection{\gls{ipfs}}
        \paragraph{}
        \gls{ipfs}, being distributed, is resilient to single point of failure. It is more secure to Denial of Service (DDoS) attacks than centralized system. Multiple nodes can simultaneously participate in distributing a particular file resulting in better throughput. The data retrieved from \gls{ipfs} can be verified by calculating its hash. Hence, data stored on \gls{ipfs} is self-verifiable. Data can be pinned on multiple nodes to ensure it persists on \gls{ipfs}. This makes \gls{ipfs} resilient to data loss.
...
\section{Proposed Solution Architecture}
    ...
    \begin{itemize}
        ...
        \item \textbf{\gls{ipfs} :-} Since blockchain network is not ideal for storing files, \gls{ipfs} is used to store files such as certificates and scanned documents.
    ...
    \subsection{Records Server}
        \paragraph{}
        This serves a web application that allows users in Land Registration Department to enter and query land records on blockchain. To support migration of old land records onto the new platform, uploading scanned documents with each transaction such as adding a land record, transferring a land or splitting a land record is supported. All the documents are stored in the \gls{ipfs}.

        \paragraph{}
        For each land record, a PDF certificate, digitally signed by the Land Registration Department, is generated which includes all the information about the land such as the current owner, khasra number etc. This certificate is stored in the \gls{ipfs}.
        
        \paragraph{}
        For each land transfer, a PDF certificate, digitally signed by the Land Registration Department, is generated which includes all the information related to the land transfer. This certificate is stored in the \gls{ipfs}. In a land transfer, land record certificate is also updated to reflect the current owner.
        ...
        \subsubsection{Add Land Record}
            ...
            \begin{enumerate}
                ...
                \item Upload certificate to \gls{ipfs} and receive \gls{cid} for the certificate.
        ...
        \subsubsection{Transfer Land}
            ...
            \begin{enumerate}
                ...
                \item Upload certificates to \gls{ipfs} and receive \gls{cid}'s for the certificates.
    ...
    \subsection{\gls{lrrs} Server}
        ...
        \paragraph{}
        The generated PDF contains general details about the land parcel on top and it also includes a Verification Code which can be used to verify the current owner of the land parcel using the Verification server and selecting the record type as Land Record. The PDF also contains a table of ownership history of the land parcel from genesis with the Verification Code for the Transfer Records which can also be verified on the verification server. These verification codes are the hash ID's of the records stored on the \gls{ipfs}. To validate the land record and the transfer records, the users can request for these certificates from the verification portal.
        \begin{figure}[H]
    ...
    \subsection{Verification Server}
        ...
        \begin{enumerate}
            \item Get input from the user which includes \gls{ipfs} hash of the certificate and record details.
            \item Fetch the corresponding record from the Blockchain network.
            \item Validate the provided certificate \gls{ipfs} hash against the one associated with the record in the Blockchain.
            \item If certificate hashes matched, fetch the stored certificate from \gls{ipfs}.
    ...
    \subsection{Blockchain}
        ...
        \begin{itemize}
            ...
            \item \textbf{certificate} - \gls{cid} of the certificate file stored in \gls{ipfs}
            \item \textbf{otherDocs} - Array of \gls{cid}'s of the files uploaded to \gls{ipfs} with the land record
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \item \textbf{certificate} - \gls{cid} of the certificate file stored in \gls{ipfs}
            \item \textbf{otherDocs} - Array of \gls{cid}'s of the files uploaded to \gls{ipfs} with the land transfer record
        \end{itemize}
...
\section{Deployment Details}
    \subsection{Records Server}
        ...
        \begin{itemize}
            ...
            \item \textbf{IPFS\_CLUSTER :-} Link to \gls{ipfs} cluster node
        \end{itemize}
    ...
    \subsection{\gls{ipfs}}
        \paragraph{}
        A private \gls{ipfs} network is set up. IPFS-Cluster is used to manage the cluster. This requires two environment variables to be provided :-
        \begin{itemize}
            \item \textbf{SWARM\_KEY :-} Required for private \gls{ipfs} network, can be generated by following point 2 in this \href{https://github.com/ahester57/ipfs-private-swarm}{link}.
            \item \textbf{CLUSTER\_SECRET :-} Required to manage the private \gls{ipfs} cluster
        \end{itemize}
        ...
        \begin{lstlisting}[language=bash, caption=\gls{ipfs}]
