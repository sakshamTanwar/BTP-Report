...
\section{Chapter 4}
    ...
    \paragraph{}
    The smartphone application is built using React Native a NodeJS module, It is written in Typescript. To build Android APK, Linux, Windows and macOS are the supported development OS but to build iOS IPA macOS is required.
    ...
    \begin{itemize}
        ...
        \item If you are not already using Hyper-V: Performance (Intel ® HAXM)
    \end{itemize}
    ...
    \paragraph{}
    To build a release version of the app for iOS, an Apple developer account is required. After signing in using the developer account in the Xcode, a build button is available which builds a release IPA for iOS and pushes it to Apple Servers for verification for deployment to the App Store.
    ...
    # Modify init.sh file permissions
    chmod 740 init.sh
